 ...
 * Verifies save/load cycle with AES-256-GCM encryption
 */
...
// Set up isolated temp directory for test store
...
process.env.ENDPOINT_STORE_SECRET = 'test-secret-key-for-e2e-testing';
...
try {
  ...
  // Verify the file is encrypted (not plaintext JSON)
  const { readFileSync } = await import('node:fs');
  const raw = readFileSync(storePath, 'utf8');
  assert.ok(raw.startsWith('v1:'), 'stored data uses versioned envelope');
