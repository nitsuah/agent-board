...
async function testContentGen() {
  ...
  try {
    ...
    if (!text) {
      ...
      const hasToolRef = /script|video|gen|audio|content|voice|image|visual/i.test(text);
      if (!hasToolRef) {
        recordFail(NAME, `response missing tool names — got: ${text.slice(0, 120)}`);
      } else {
...
async function testWebsiteAgent() {
  ...
  try {
    ...
    if (!text) {
      ...
      const hasToolRef = /lead|site|deploy|file|netlify|discover|pitch|web/i.test(text);
      if (!hasToolRef) {
        recordFail(NAME, `response missing tool names — got: ${text.slice(0, 120)}`);
      } else {
        console.log(`  response snippet: ${text.slice(0, 120)}`);
